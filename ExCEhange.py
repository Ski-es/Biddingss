...
with st.sidebar:
    selected= option_menu(
        ...
        options= ["Profile", "Home","Construction Projects", "Bidding", "About", "FAQS", "Feedback", "Report", "Code of Conduct", "Contacts"])
        ...
        if st.button("Register"):
            ...
            try:
                cursor.execute("INSERT INTO users (username, hashed_password) VALUES (?, ?)",
                               (register_username, hashed_password))
                conn.commit()
...
elif selected== "Construction Projects":
...
# Bidding page
    ...
    selected_project_name = st.selectbox("Select a Project", projects_df["name"])
    bidder_name = st.text_input("Your Name")
    bid_amount = st.number_input("Bid Amount", min_value=0)

    if st.button("Place Bid"):
        selected_project_id = projects_df[projects_df["name"] == selected_project_name]["id"].values[0]
        place_bid(selected_project_id, bidder_name, bid_amount)
        st.success(f"{bidder_name} placed a bid of ${bid_amount} for '{selected_project_name}'")

    # Display bids for a selected project
    st.header("Bids for Selected Project")
    selected_project_name_view = st.selectbox("Select a Project to View Bids", projects_df["name"])

    if selected_project_name_view:
        selected_project_id_view = projects_df[projects_df["name"] == selected_project_name_view]["id"].values[0]
        project_details, bids = get_project_details(selected_project_id_view)
